...
import { Search, ChevronDown, ChevronRight, Calculator, DollarSign, CheckCircle2, Clock, AlertCircle } from "lucide-react"
...
export default function PagosPage() {
  ...
  async function loadData() {
    try {
      const { data: sociosData } = await supabase.from("socios").select("*").order("certificado_no")
      const { data: planesData } = await supabase.from("planes_pago").select("*")
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Proyección de cuotas con interés del {tasaInteres}% mensual</p>
        </div>
